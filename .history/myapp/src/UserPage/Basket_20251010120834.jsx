...
export default function Basket() {
  ...
  return (
<div className="min-h-screen min-w-screen flex flex-col lg:flex-row bg-gray-900 text-white p-4 lg:p-6 gap-4 lg:gap-6">
  {/* Sol panel */}
  <div className="flex-1 flex flex-col gap-3 overflow-y-auto max-h-screen">
    {basket.items.map((item) => (
      ...
      >
        {/* Şəkil və ad */}
        <div className="flex items-center gap-2 w-full sm:w-1/3 mb-1 sm:mb-0">
          <img
          ...
          />
          <div>
            <h2 className="font-bold text-sm">{item.product.name}</h2>
            <p className="text-gray-300 text-xs">${item.product.price}</p>
          </div>
        </div>

        {/* Quantity */}
        <div className="flex items-center gap-1 w-full sm:w-1/3 justify-center mb-1 sm:mb-0">
          <button
            onClick={() =>
        ...
        {/* Sil */}
        <div className="flex flex-col items-center gap-1 w-full sm:w-1/3 justify-end">
          <button
  ...
  {/* Sağ panel */}
  <div className="w-full max-w-[280px] bg-white/10 backdrop-blur-lg p-3 rounded-2xl flex flex-col gap-3 self-start sticky top-4">
    <h2 className="text-lg font-bold text-red-400 text-center">Səbət Xülasəsi</h2>
    ...
    <div className="flex flex-col gap-2 max-h-64 overflow-y-auto">
      {basket.items.map((item) => (
        <div key={item.id} className="flex justify-between items-center text-sm">
          ...
          <span className="text-red-400">${(item.product.price * item.quantity).toFixed(2)}</span>
        </div>
      ))}
    </div>
