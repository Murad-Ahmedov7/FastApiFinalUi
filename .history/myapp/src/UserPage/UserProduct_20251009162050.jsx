...
<div className="relative z-10 p-6 max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
  {products.length === 0 ? (
    ...
    products.map((product) => (
      <div
        key={product.id}
        className="bg-gradient-to-br from-red-500 to-black-700 text-white rounded-xl shadow-lg p-6 flex flex-col justify-between transform transition-transform duration-300 hover:-translate-y-2 hover:shadow-2xl"
      >
        {/* Şəkil */}
        <img
          src={product.image || product.name=='laptop' && "/images/laptop.png"} // internetdən şəkil
          alt={product.name}
          className="w-full h-40 object-cover rounded-md mb-4 botder-3"
        />
