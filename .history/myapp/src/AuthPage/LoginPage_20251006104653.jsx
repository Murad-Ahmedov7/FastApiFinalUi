...
const res = await axios.post(`${API_URL}/users/login`, {      email: email,
  ...
  const handleSelectRole = (role) => {
    console.log('buttona clicklendi')
    setIsInputDisabled(false)
    setActiveRole(role)
      console.log("Role:", role, "Disabled:", isInputDisabled); // dərhal dəyişməyəcək
  }
  ...
  return (
    <div className="flex items-center justify-center overflow-y-hidden  h-screen w-screen relative">
      ...
      <div className="flex justify-center items-center h-full absolute gap-3 top-[300px]  ">
        ...
        <button onClick={() => handleSelectRole("user")} className="text-white w-30 bg-gradient-to-r from-red-300 via-red-500 to-red-400 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-md px-5 py-2.5 text-center me-2 mb-2 ">
          User
        </button>
      </div>
    </div>
